    ...
    public static Field backtrack(Field state, Player player) {
        ...
        for (Field move : moves) {
            if (Field.countPieces(move, opponent) < Field.countPieces(state, opponent)) {
                ...
                Arrays.sort(moves2, (m1, m2) -> Double.compare(score(m2, player, player), score(m1, player, player)));
        ...
        Field nextGenMove = new CnnAlphaBetaAI(2, player).bestMove(state);
        if (nextGenMove != null) {
