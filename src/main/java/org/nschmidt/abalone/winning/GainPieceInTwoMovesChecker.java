...
public enum GainPieceInTwoMovesChecker {
    INSTANCE;
    
    public static Field[] gainPieceInTwoMoves(Field state, Player player) {
        ...
        return Arrays.asList(firstMoves).parallelStream().map(firstMove -> {
            ...
            for (Field opponentMove : opponentMoves) {
                ...
                for (Field secondMove : secondMoves) {
                    ...
                    hasSolution = !gainPiece(secondMove, opponent);
                    ...
                    for (Field lastOpponentMove : lastOpponentMoves) {
                        if (!gainPiece(lastOpponentMove, player)) {
                            hasSolution = false;
                            break;
                        }
                    }
