...
class TicTacToeZeroBuilder {
    ...
    private final int fieldWidthAndHeight;
    
    public TicTacToeZeroBuilder(int[] kernel, int[] strides, ConvolutionMode mode, int fieldWidthAndHeight, int numPlanes) {
        this.fieldWidthAndHeight = fieldWidthAndHeight;
        ...
        this.conf =  new NeuralNetConfiguration.Builder()
            ...
            .graphBuilder().setInputTypes(InputType.convolutional(fieldWidthAndHeight, fieldWidthAndHeight, numPlanes));
    }

    public TicTacToeZeroBuilder() {
        this(new int[] {3, 3},  new int[] {1, 1}, ConvolutionMode.Same, 3, 5);
    }
     ...
     */
    public String addPolicyHead(String inName) {
    ...
    public String addValueHead(String inName) {
        ...
        conf.addLayer(actName, new ActivationLayer.Builder().activation(Activation.RELU).build(), bnName);
        conf.addLayer(denseName, new DenseLayer.Builder().nIn(fieldWidthAndHeight * fieldWidthAndHeight).nOut(256).build(), actName);
        Map<String, InputPreProcessor> preProcessorMap = new HashMap<>();
        preProcessorMap.put(denseName, new CnnToFeedForwardPreProcessor(fieldWidthAndHeight, fieldWidthAndHeight, 1));
        conf.setInputPreProcessors(preProcessorMap);
